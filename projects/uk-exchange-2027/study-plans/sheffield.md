...
- **EGB241 → ELE343 Electromagnetic Fields and Devices** — EGB241 covers *both* fields **and** machines/transformers/3-phase power (per its QUT description); ELE343's description only mentions fields (Maxwell's equations, static field problems), not machines. This may not clear 80% alone. No single catalogued module found that covers both halves — worth getting ELE343's full outline to check, or asking faculty whether combining it with a machines-focused backup (e.g. ELE335 Machine Design) is an option for one QUT unit.
- **EGB240 → ELE339 Digital Engineering** — EGB240 is a broad practical circuit-design project unit (CAD tools, build/test/document simple circuits); ELE339 is specifically digital processor organisation/architecture, a narrower sub-topic. Worth the same full-outline check.
